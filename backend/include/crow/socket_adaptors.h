...
namespace crow
{
...
#else
    using error_code = boost::asio::error_code;
#endif
    using tcp = boost::asio::ip::tcp;
    using stream_protocol = boost::asio::local::stream_protocol;

    /// A wrapper for the boost::asio::ip::tcp::socket and asio::ssl::stream
    ...
    {
        using context = void;
        UnixSocketAdaptor(asio::io_context &io_context, context *) : socket_(io_context)
        {
        }
